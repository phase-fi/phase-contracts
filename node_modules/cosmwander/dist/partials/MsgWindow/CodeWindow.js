...
const CodeWindow = () => {
  ...
  const sendInitMsg = i => {
    if (contract && contract.fileName && codeId && env) {
      ...
      setCommand({
        command: envConfig.command,
        args: ['tx', 'wasm', 'instantiate', codeId, msg.msg, '--from', envConfig.keyName, '--label', msg.title, '--gas-prices', envConfig.feeAmount + envConfig.feeDenom, '--gas', 'auto', '--gas-adjustment', '1.3', '-b', 'block', '-y', '--no-admin', '--node', envConfig.node, '--chain-id', envConfig.chainId, ...(funds ? ['--amount', funds] : [])],
        cwd: (0, fileUtils_1.getCWD)(),
  ...
  return (0, jsx_runtime_1.jsxs)("box", {
    ...
    children: [(0, jsx_runtime_1.jsx)("text", {
      left: 1,
      children: chalk.bold(`Code ID: ${codeId} Init Msg`) + chalk.gray(' (press esc if stuck)')
